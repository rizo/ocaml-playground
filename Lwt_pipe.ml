...
type 'a read_result = Closed | Timeout | Value of 'a

type 'a reader = { r_wakeup : 'a read_result Lwt.u; mutable r_timeout : bool }
...
type ('a, +'perm) t = {
  ...
  readers : 'a reader Queue.t;
  (* blocked readers *)
  buf : 'a Queue.t;
  (* internal buffers of written values *)
  blocked_writers : ('a * bool Lwt.u) Queue.t;
  (* blocked writers *)
  max_size : int;
...
let close_ p =
  if not p.stopped then (
    ...
    Queue.iter (fun { r_wakeup; _ } -> Lwt.wakeup r_wakeup Closed) p.readers;
    Queue.iter (fun (_, r) -> Lwt.wakeup r false) p.blocked_writers;
    if Queue.length p.buf = 0 then Lwt.wakeup p.close () (* close immediately *))


let close_nonblock p = if not p.stopped then close_ p

let close p =
  if not p.stopped then (
    close_nonblock p;
    p.closed >>= fun () -> Lwt.join p.keep)
  else p.closed
...
let read_ t ~timeout : 'a read_result Lwt.t =
  ...
  if not (Queue.is_empty t.buf) then (
    ...
    if
      (not (Queue.is_empty t.blocked_writers))
      && Queue.length t.buf < t.max_size
    then (
...
let read_with_timeout t ~timeout = read_ t ~timeout

(*$= read_with_timeout
  (read_with_timeout ~timeout:(Some 0.001) (create ()) |> Lwt_main.run)  (Timeout)
  (read_with_timeout ~timeout:(Some 0.001) (of_list [1]) |> Lwt_main.run) (Value 1)
  (read_with_timeout ~timeout:(Some 0.001) (of_list []) |> Lwt_main.run) (Closed)
*)
...
let read t = read_ t ~timeout:None >|= opt_of_available
...
let rec write_rec_ (t : ('a, _) pipe) (x : 'a) =
  if t.stopped then Lwt.return_false
...
let rec connect_rec r w =
  ...
  | Value x ->
    write_rec_ w x >>= fun ok -> if ok then connect_rec r w else Lwt.return_unit
  | _ -> Lwt.return_unit


(* close a when b closes *)
let link_close p ~after =
  Lwt.on_termination after.closed (fun _ -> close_nonblock p)
...
let write_exn t x =
  write_rec_ t x >>= fun ok -> if ok then Lwt.return_unit else Lwt.fail Closed
...
let rec write_list t l =
  match l with
  | [] -> Lwt.return true
  | x :: tail ->
    let fut = write t x in
    fut >>= fun b -> if b then write_list t tail else fut


let write_list_exn t l =
  write_list t l >>= fun ok -> if ok then Lwt.return_unit else Lwt.fail Closed


let to_stream p = Lwt_stream.from (fun () -> read p)

let of_stream s =
  let p = create () in
  let rec send s =
    Lwt_stream.get s >>= fun result ->
    match result with
    | None -> Lwt.return_unit
    | Some elt ->
      write p elt >>= fun ok -> if ok then send s else Lwt.return_unit
  in
  let fut = send s in
  keep p fut;
...
module Writer = struct
  ...
  let map ~f a =
    ...
    let rec fwd_rec () =
      read b >>= function
      | Some x ->
        write a (f x) >>= fun ok -> if ok then fwd_rec () else Lwt.return_unit
      | None -> Lwt.return_unit
    in
    let fwd = fwd_rec () in
    keep b fwd;
...
module Reader = struct
  ...
  let map ~f a =
    ...
    let rec fwd_rec () =
      read a >>= function
      | Some x ->
        write b (f x) >>= fun ok -> if ok then fwd_rec () else Lwt.return_unit
      | None -> Lwt.return_unit
    in
    let fwd = fwd_rec () in
    keep b fwd;
  ...
  *)
  ...
  let map_s ~f a =
    ...
    let rec fwd_rec () =
      ...
      | Some x ->
        f x >>= fun y ->
        write b y >>= fun ok -> if ok then fwd_rec () else Lwt.return_unit
      | None -> Lwt.return_unit
    in
    let fwd = fwd_rec () in
    keep b fwd;
  ...
  *)
  ...
  let filter ~f a =
    ...
    let rec fwd_rec () =
      ...
      | Some x ->
        if f x then
          write b x >>= fun ok -> if ok then fwd_rec () else Lwt.return_unit
        else fwd_rec ()
      | None -> Lwt.return_unit
    in
    let fwd = fwd_rec () in
  ...
  *)
  ...
  let flat_map ~f a =
    ...
    let rec fwd_rec () =
      ...
      | Some x ->
        let l = f x in
        write_list b l >>= fun ok -> if ok then fwd_rec () else Lwt.return_unit
      | None -> Lwt.return_unit
    in
    let fwd = fwd_rec () in
    keep b fwd;
  ...
  *)
  ...
  let rec fold_s ~f ~x t =
    read t >>= function
    | None -> Lwt.return x
    | Some y -> f x y >>= fun x -> fold_s ~f ~x t
  ...
  *)
  ...
  let merge_all l =
    ...
    res.keep <- conns;
    Lwt.async (fun () -> Lwt.join conns >>= fun () -> close res);
    res
...
let of_list l : _ Reader.t =
  ...
  let rec send = function
    | [] -> Lwt.return_unit
    | h :: t -> write p h >>= fun ok -> if ok then send t else Lwt.return_unit
  in
  let fut = send l in
  keep p fut;
  Lwt.on_termination fut (fun () -> close_nonblock p);
...
let of_array a =
  ...
  let rec send i =
    if i < Array.length a then
      write p a.(i) >>= fun ok -> if ok then send (i + 1) else Lwt.return_unit
...
let of_lwt_klist l =
  ...
  let rec next l =
    ...
    | `Nil -> Lwt.return_unit
    | `Cons (x, tl) ->
      write p x >>= fun ok -> if ok then next tl else Lwt.return_unit
  in
  let fut = next l in
  keep p fut;
  Lwt.on_termination fut (fun () -> close_nonblock p);
...
let to_list_rev r = Reader.fold ~f:(fun acc x -> x :: acc) ~x:[] r
...
let to_buffer buf r = Reader.iter ~f:(fun c -> Buffer.add_char buf c) r

let to_buffer_str ?(sep = "") buf r =
  let first = ref true in
  Reader.iter r ~f:(fun s ->
...
module IO = struct
  ...
  let read_lines ic =
    ...
    let rec send () =
      ...
      | None -> Lwt.return_unit
      | Some line ->
        write p line >>= fun ok -> if ok then send () else Lwt.return_unit
    in
    let fut = send () in
    keep p fut;
    Lwt.on_termination fut (fun () -> close_nonblock p);
  ...
  let write oc =
    let p = create () in
    let fut =
      Reader.iter_s ~f:(Lwt_io.write oc) p >>= fun _ -> Lwt_io.flush oc
    in
    keep p fut;
    Lwt.on_termination fut (fun () -> close_nonblock p);
    p
