(** This modules provides utilities for constructing and generating HTML content.

    {b Note}: Whenever an element or attribute name conflicts with an OCaml keyword we
    prime it, see for example {!type'}. *)

(** {1:attr Attributes}

    See the {{:https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes} MDN HTML
    attribute reference}. *)
...
(** [class_flags list] is similar to {!class_list}, but can conditionally omit class names
    depending on the boolean values in [list]. *)
...
module Attr : sig
  ...
  (** [on_some option] is [attr] if [option] is [Some attr] and {!empty} if [option] is
      [None]. *)

  val on_ok : (attr, 'e) result -> attr
  (** [on_ok result] is [attr] if [result] is [Ok attr] and {!empty} if [result] is
      [Error _]. *)
end
...
val doctype : html
(** [doctype] is the ["<!DOCTYPE html>"] element. *)
...
module Elem : sig
  ...
  val empty : html
  (** [empty] is an element that doesn't get rendered. *)

  val fmt : ('a, Format.formatter, unit, unit, unit, t) format6 -> 'a
  (** [fmt format] is similar to {!text}, but creates character data from a formatted
      string. *)

  val raw : string -> html
  (** [raw html_string] is the raw HTML string [html_string] without escaping markup
      delimiters. [html_string] must be well-formed HTML otherwise invalid markup will be
      generated.

      {b Warning}: Using this function is discouraged! *)

  val of_some : ('a -> html) -> 'a option -> html
  (** [of_some to_html option] is [to_html x] if [option] is [Some x] and [empty]
      otherwise. *)
...
(** {1:head Head elements} *)

module Head : sig
  val title : attr list -> html list -> html
  (** See {{:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/title} title}. *)

  val style : attr list -> html list -> html
  (** See {{:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/style} style}

      {b Warning}: If your style htmlent holds CSS use {!Elem.raw} content, otherwise the
      CSS selector [>] gets escaped. *)

  val body : attr list -> html list -> html
  (** See {{:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/body} body}. *)

  val link : attr list -> html
  (** See {{:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/link} link}. *)

  val script : attr list -> html list -> html
  (** See {{:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/script} script}. *)

  val noscript : attr list -> html list -> html
  (** See {{:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/noscript}
      noscript}. *)

  val template : attr list -> html list -> html
  (** See {{:https://developer.mozilla.org/en-US/docs/Web/HTML/Element/template}
      template}. *)
end

(** {1:out Output generation} *)

val to_string : html -> string
(** [to_string html] is the string representation of [html]. *)

val write_to_channel : out_channel -> html -> unit
(** [write_to_channel chan html] writes the string representation of [html] to [chan]. *)

val write_to_buffer : Buffer.t -> html -> unit
(** [write_to_buffer buf html] writes the string representation of [html] to [buf]. *)
